...
- Settings changes apply via a full backend restart (Docker's `restart: unless-stopped` picks up the new config on next boot) — no live-reload plumbing.
...
### Task 4: Wire settings + asset store into startup (`main.go`, `Deps`)
...
- Produces: `Deps.AssetStore renderstore.RenderStore` (new field) — Task 5 and Task 6 write/read through it. A standalone, testable function `loadOrSeedStorageSettings(ctx, *store.Store, *config.Config) (*model.StorageSettings, error)` in a new small file so its seed-on-first-boot logic has a unit test without spinning up the whole server.
...
type Deps struct {
	...
	AssetStore    renderstore.RenderStore
}
```

- [ ] **Step 3: Write the failing test for the seed-or-load helper**
...
- [ ] **Step 5: Write the seed-or-load helper**
...
Then update the `Deps` construction (which currently does `Store: store.New(database),`) to reuse `appStore` and add `AssetStore: as`:
...
- [ ] **Step 8: Verify the whole backend builds and every test passes**
...
```bash
git add backend/internal/config/config.go backend/internal/api/deps.go backend/cmd/server/main.go backend/cmd/server/storage_boot.go backend/cmd/server/storage_boot_test.go
git commit -m "feat: load storage settings from DB (env-seeded) and build an encrypted asset store at boot"
```
...
- `newTestDepsWithCache(t) *api.Deps` (in `sync_test.go`) builds `Deps` via `newTestDeps(t)` plus `Cache: gitcache.NewCache(t.TempDir())` and `RenderStore: renderstore.NewLocal(t.TempDir())` — it does **not** set `AssetStore`, so this task's test must set it explicitly.
...
func TestHandleSync_writesAssetsToAssetStore(t *testing.T) {
	...
	deps.AssetStore = renderstore.NewLocal(t.TempDir())
	ctx := context.Background()
...
- [ ] **Step 4: Implement the write-through**
...
### Task 7: Admin API — get/update storage settings with validation + restart-on-save
...
- Consumes: `store.GetStorageSettings`/`UpsertStorageSettings` (Task 3), `renderstore.New` (Task 2, for the validation round-trip), `requireAdmin` (existing, `admin.go`).
...
Add `"github.com/pubobs/backend/internal/model"` to the imports above (used in the `UpsertStorageSettings` calls).
...
func handleAdminUpdateStorageSettings(deps *Deps) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		// Exit after the response is flushed so Docker's restart policy picks
		// up the new settings on the next boot. See Global Constraints: this
		// project deliberately restart-to-apply rather than live-reload.
		go func() {
...
// candidate settings before they're persisted, so a bad bucket/credential
// is caught immediately rather than bricking the instance on next boot.
...
Add `"os"`, `"time"`, and a package-level `var errNotFound = errors.New("validation object mismatch")` (with `"errors"` imported) at the top of the file.
...
Run: `cd backend && go test ./internal/api/... -run TestAdminStorageSettings -v`
Expected: PASS. (The `TestAdminStorageSettings_putRejectsInvalidS3Config` case relies on `127.0.0.1:1` refusing the connection immediately — if this is flaky in CI, swap for a syntactically-valid-but-unreachable endpoint like `s3.invalid.invalid:9000`, which fails DNS resolution immediately instead of waiting on a TCP timeout.)
...
git commit -m "feat: add admin API for storage settings (get/update with S3 validation + restart-on-save)"
```
...
- Modify: `backend/internal/api/router.go`
- Test: `backend/internal/renderstore/s3_prefix_test.go` (add coverage for key-building only — real S3 listing needs a live/mocked endpoint, see note below)
- Test: `backend/internal/api/admin_storage_test.go`

**Interfaces:**
- Consumes: `gitcache.Cache.DiskUsage()` (existing), `os` directory-size walking (new small helper), `S3RenderStore` (Task 2).
...
There's no existing test infrastructure for a live S3-compatible server in this repo (no MinIO test container, no mocked HTTP transport for `minio-go`), so the S3 listing method itself is exercised only via the type-level test below (confirms it compiles and the prefix-matching logic is correct against a fabricated `minio.ObjectInfo` slice by extracting that filtering into a small, directly-testable pure function) — not an end-to-end network test. This matches the project's existing testing depth for S3 code (`s3.go` itself has no direct unit tests today; only `store_test.go`'s local-store tests and the new prefix test from Task 2 touch it).
...
- [ ] **Step 5: Add a local directory-size helper and the admin handler**
...
func handleAdminStorageUsage(deps *Deps) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		if s3Renders, ok := deps.RenderStore.(*renderstore.S3RenderStore); ok {
...
- [ ] **Step 7: Write and run a handler-level test**

`newTestDeps(t)`'s `Config` only sets `SecretKey`/`BaseURL` — this test needs `RepoCacheDir`/`RenderDir`/`AssetDir` to point at real temp directories too, since `dirSizeBytes` walks them directly.
...
func TestAdminStorageUsage_reportsLocalBreakdown(t *testing.T) {
	...
	deps.RenderStore = renderstore.NewLocal(deps.Config.RenderDir)
	deps.AssetStore = renderstore.NewLocal(deps.Config.AssetDir)
...
```bash
git add backend/internal/renderstore/s3.go backend/internal/renderstore/s3_prefix_test.go backend/internal/api/admin_storage.go backend/internal/api/admin_storage_test.go backend/internal/api/router.go
git commit -m "feat: add admin API for local + S3 disk usage breakdown"
```
...
func TestAdminStorageMigrate_startsAndReportsProgress(t *testing.T) {
	...
	deps.RenderStore = renderstore.NewLocal(t.TempDir()) // stands in for "the S3 destination"
	deps.AssetStore = renderstore.NewLocal(t.TempDir())
...
// point at (the settings the admin just saved and the server restarted
// into) — i.e. it's always "local → whatever's configured now", matching
// this feature's actual use case of moving *off* local disk.
...
- [ ] **Step 5: Update the test to configure S3 first, or adjust the guard**

Since Step 3's guard now rejects migration while `store_type == "local"`, update `TestAdminStorageMigrate_startsAndReportsProgress` from Step 1 to first seed `storage_settings` with `store_type = "s3"` pointing at a real reachable test target — but per Task 8's note, there's no live S3 test fixture in this repo. Instead, directly construct `deps.RenderStore`/`deps.AssetStore` in the test as a *second* local store (simulating "S3" for test purposes, since `RunMigrationCycle` only cares about the `RenderStore` interface, not the concrete backend) and set `settings.StoreType = "s3"` purely to satisfy the guard, e.g.:

```go
	// Simulate "migrated to a different backend" using a second local dir —
	// RunMigrationCycle only depends on the RenderStore interface, so this
	// exercises the same code path a real S3 destination would.
	settings, _ := deps.Store.GetStorageSettings(context.Background())
	settings.StoreType = "s3"
	require.NoError(t, deps.Store.UpsertStorageSettings(context.Background(), settings))
```

Add this to the test's setup before the `doAuthedPost` call.
...
export async function storageSettingsView(): Promise<HTMLElement> {
  ...
  hint.textContent = 'Render blobs and media assets can be stored locally or on S3-compatible object storage. Changing this restarts the backend.';
...
function buildForm(settings: StorageSettings, onSaved: () => Promise<void>): HTMLElement {
  ...
  saveBtn.textContent = 'Save (restarts backend)';
...
async function waitForBackendRestart(): Promise<void> {
  await new Promise(resolve => setTimeout(resolve, 500)); // give the process time to actually exit
  for (let i = 0; i < 30; i++) {
    try {
